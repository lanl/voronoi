...
subroutine MergeGrids(grid1,grid2)
#include "petsc/finclude/petscvec.h"
  use petscvec
...
!===========================================
  ...
  if (rank == io_rank) then
    ...
    do ielem = 1, grid%num_elems_global
      ...
      if (grid%lg_flag .eqv. PETSC_TRUE) then
        if (grid%ndim == 2) then
          temp_int = (/ielem,int(itet(3*(ielem-1)+1)),int(itet(3*(ielem-1)+2)),int(itet(3*(ielem-1)+3))/)
        else
          temp_int = (/ielem,int(itet(4*(ielem-1)+1)),int(itet(4*(ielem-1)+2)),int(itet(4*(ielem-1)+4)),int(itet(4*(ielem-1)+3))/)
        endif
...
#include "petsc/finclude/petscmat.h"
  ...
  PetscErrorCode :: ierr
  PetscInt :: i
  PetscInt :: d_nnz1(grid%num_pts_local),o_nnz1(grid%num_pts_local)
...
#include "petsc/finclude/petscmat.h"
  ...
  type(grid_type) :: grid
  type(diag_atts) :: atts
  PetscInt  :: i,idx,iproc,temp(3),rank,irank
  PetscInt  :: one=1
  ...
  do i=1,grid%num_elems_local
    ...
    ! dump areas/lengths
    call MatSetValue(grid%adjmatrix,min0(temp(1),temp(2))-1,max0(temp(1),temp(2))-1,cell_area(1)/cell_len(1),ADD_VALUES,ierr);CHKERRQ(ierr)
    call MatSetValue(grid%adjmatrix,min0(temp(2),temp(3))-1,max0(temp(2),temp(3))-1,cell_area(2)/cell_len(2),ADD_VALUES,ierr);CHKERRQ(ierr)
    call MatSetValue(grid%adjmatrix,min0(temp(1),temp(3))-1,max0(temp(1),temp(3))-1,cell_area(3)/cell_len(3),ADD_VALUES,ierr);CHKERRQ(ierr)

    !if ((grid%is_tough .EQV. PETSC_TRUE) .OR. (atts%are_on)) then
      call MatSetValue(grid%adjmatrix_area,min0(temp(1),temp(2))-1,max0(temp(1),temp(2))-1,cell_area(1),ADD_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_area,min0(temp(2),temp(3))-1,max0(temp(2),temp(3))-1,cell_area(2),ADD_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_area,min0(temp(1),temp(3))-1,max0(temp(1),temp(3))-1,cell_area(3),ADD_VALUES,ierr);CHKERRQ(ierr)
      ...
      call MatSetValue(grid%adjmatrix_len,min0(temp(1),temp(2))-1,max0(temp(1),temp(2))-1,cell_len(1)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_len,min0(temp(2),temp(3))-1,max0(temp(2),temp(3))-1,cell_len(2)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_len,min0(temp(1),temp(3))-1,max0(temp(1),temp(3))-1,cell_len(3)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
    !endif
...
#include "petsc/finclude/petscmat.h"
  ...
  type(grid_type) :: grid
  type(diag_atts) :: atts
  PetscInt  :: i,idx,iproc,rank,irank
  PetscInt  :: one=1
  ...
  do i=1,grid%num_elems_local
     ...
     call MatSetValue(grid%adjmatrix,min0(temp(2),temp(3))-1,max0(temp(2),temp(3))-1,cell_area(2)/cell_len(2),ADD_VALUES,ierr);CHKERRQ(ierr)
     call MatSetValue(grid%adjmatrix,min0(temp(1),temp(3))-1,max0(temp(1),temp(3))-1,cell_area(3)/cell_len(3),ADD_VALUES,ierr);CHKERRQ(ierr)
     call MatSetValue(grid%adjmatrix,min0(temp(1),temp(4))-1,max0(temp(1),temp(4))-1,cell_area(4)/cell_len(4),ADD_VALUES,ierr);CHKERRQ(ierr)
     call MatSetValue(grid%adjmatrix,min0(temp(2),temp(4))-1,max0(temp(2),temp(4))-1,cell_area(5)/cell_len(5),ADD_VALUES,ierr);CHKERRQ(ierr)
     call MatSetValue(grid%adjmatrix,min0(temp(3),temp(4))-1,max0(temp(3),temp(4))-1,cell_area(6)/cell_len(6),ADD_VALUES,ierr);CHKERRQ(ierr)
     ...
     !if ((grid%is_tough .EQV. PETSC_TRUE) .OR. (atts%are_on .EQV. PETSC_TRUE)) then
      ...
      call MatSetValue(grid%adjmatrix_area,min0(temp(2),temp(3))-1,max0(temp(2),temp(3))-1,cell_area(2),ADD_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_area,min0(temp(1),temp(3))-1,max0(temp(1),temp(3))-1,cell_area(3),ADD_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_area,min0(temp(1),temp(4))-1,max0(temp(1),temp(4))-1,cell_area(4),ADD_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_area,min0(temp(2),temp(4))-1,max0(temp(2),temp(4))-1,cell_area(5),ADD_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_area,min0(temp(3),temp(4))-1,max0(temp(3),temp(4))-1,cell_area(6),ADD_VALUES,ierr);CHKERRQ(ierr)
      ...
      call MatSetValue(grid%adjmatrix_len,min0(temp(2),temp(3))-1,max0(temp(2),temp(3))-1,cell_len(2)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_len,min0(temp(1),temp(3))-1,max0(temp(1),temp(3))-1,cell_len(3)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_len,min0(temp(1),temp(4))-1,max0(temp(1),temp(4))-1,cell_len(4)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_len,min0(temp(2),temp(4))-1,max0(temp(2),temp(4))-1,cell_len(5)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
      call MatSetValue(grid%adjmatrix_len,min0(temp(3),temp(4))-1,max0(temp(3),temp(4))-1,cell_len(6)/2.d0,INSERT_VALUES,ierr);CHKERRQ(ierr)
    !endif
...
!  calculate the positive normal direction of the element if no longer on the same fracture
  ...
  ! 
  ! TMP
  ! This is an attempt to create Voronoi polygons
  !
  !=============================================================================
...
#include "petsc/finclude/petscvec.h"
  ...
  PetscReal,dimension(4) :: tmp1,tmp2,tmp3,tmp4,vsum
...
! form the upper triangular sparse matrix
  do i=1,grid%num_pts_local
    ...
    call MatSetValues(grid%connectivity,one,(/i-1/)+istart_mat1,ncols,(/(j,j=1,ncols)/),temp_cols,INSERT_VALUES,ierr);CHKERRQ(ierr)
    ...
    call MatSetValues(grid%connect_area,one,(/i-1/)+istart_mat2,ncols,(/(j,j=1,ncols)/),temp_vals,INSERT_VALUES,ierr);CHKERRQ(ierr)
    ...
    call MatSetValues(grid%edgematrix,ncols-1,cols(2:ncols),one,grid%vertex_ids(i)-1,temp_cols(2:ncols),INSERT_VALUES,ierr);CHKERRQ(ierr)
    call MatSetValues(grid%adjmatrix_full,ncols-1,cols(2:ncols),one,grid%vertex_ids(i)-1,temp_vals(2:ncols),INSERT_VALUES,ierr);CHKERRQ(ierr)
    call MatSetValue(grid%adjmatrix_full,grid%vertex_ids(i)-1,grid%vertex_ids(i)-1,zero,INSERT_VALUES,ierr);CHKERRQ(ierr)
...
#include "petsc/finclude/petscmat.h"
  ...
  character(len=256) :: ioerr_msg
  PetscInt :: ioerr
  PetscInt :: fileid,i,rank,irank,size
  PetscInt :: rec_tot, rec_local, NEQ, coeff_count
  PetscInt :: io_rank = 0, num_area_coef=1
  PetscInt :: temp_int,pos1,indx1,indx2,ncon_max,ncon_max_local,mask_delta,mask_delta_local
  ...
  PetscBool, allocatable :: nil_mask(:), full_mask(:), TESTING_MASK(:)
  PetscInt :: ncols
  Vec :: mat_diag
  PetscErrorCode :: ierr
 ...
 ! deallocate(degree_diff_local)
  ...
  if (atts%coefficients_are_dedudded .eqv. PETSC_TRUE) then
    call MPI_Gather((grid%num_pts_local*3+rec_local*2+2),1,MPI_INT,sendcounts,1,MPI_INT,io_rank,MPI_COMM_WORLD,ierr);CHKERRQ(ierr)
...
56 format(5i15)
    ...
    if (grid%dump_flag .EQV. PETSC_TRUE) then
      ...
      !print*,'  Writing to: ',trim(grid%dump_str)
    else
      open(fileid,file='voronoi.stor')
      !call mpi_print("  Writing to: ./voronoi.stor",rank,io_rank)
    endif
...
#include "petsc/finclude/petscmat.h"
  ...
  PetscInt :: temp_int, temp_int2, pos1, pos2, matrix_width
  PetscInt :: io_rank = 0
  INTEGER :: hdferr
  INTEGER(HID_T) :: file, space, space2, dset, dset2, dset3 ! handles
  ...
  PetscReal, allocatable :: adj_temp_area(:),adj_temp_len(:),length_matrix(:,:)
...
#include "petsc/finclude/petscmat.h"
  ...
  PetscReal, allocatable :: areax(:),areax_tmp(:),areax_local(:),connectivity_local(:),connectivity_tmp(:)
  Vec :: mat_diag
  PetscErrorCode :: ierr
  ...
  if (rank==io_rank) vsize = maxval(connect_map)
  ...
  data_local_real(grid%num_pts_local*2+rec_local_tri+1:grid%num_pts_local*3+rec_local_tri) = (/(vec_ptr((i-1)*3+1),i=1,grid%num_pts_local)/)
  data_local_real(grid%num_pts_local*3+rec_local_tri+1:grid%num_pts_local*4+rec_local_tri) = (/(vec_ptr((i-1)*3+2),i=1,grid%num_pts_local)/)
  data_local_real(grid%num_pts_local*4+rec_local_tri+1:grid%num_pts_local*5+rec_local_tri) = (/(vec_ptr((i-1)*3+3),i=1,grid%num_pts_local)/)
...
subroutine UnitTests__EpsilonRemoval(exit_status)
  ! Unit test for epsilon removal

  implicit none

  PetscInt :: exit_status

  exit_status = 0

end subroutine UnitTests__EpsilonRemoval

subroutine UnitTests__Compression(exit_status)
  !
  ! Unit test for coefficient compression
  ! exit_status of 0 means passed, 1 is failed
  !

  implicit none

  PetscReal, allocatable :: values(:), gold(:)
  PetscInt, allocatable :: indx1(:), map(:)
  PetscInt :: N,i,j,exit_status
  ...
  values = (/0.000000000000E+00,-2.500000000000E-01,-2.500000000000E-01,&
             -2.500000000000E-01,0.000000000000E+00,2.500000000000E-01,&
             0.000000000000E+00,0.000000000000E+00,-2.500000000000E-01,&
             0.000000000000E+00,-2.500000000000E-01,0.000000000000E+00,&
             0.000000000000E+00,-2.500000000000E-01,0.000000000000E+00,&
             0.000000000000E+00,-2.500000000000E-01,-2.500000000000E-01,&
             -2.500000000000E-01,-2.500000000000E-01,0.500000000000E+00,&
             0.000000000000E+00,-2.500000000000E-01,-2.500000000000E-01,&
             0.000000000000E+00,-1.000000000000E+00,-2.500000000000E-01,&
             2.500000000000E-01,-2.500000000000E-01,-2.500000000000E-01,&
             0.000000000000E+00,0.000000000000E+00,-2.500000000000E-01,&
             1.000000000000E+00,1.000000000000E+00,-2.500000000000E-01,&
             1.000000000000E+00,-2.500000000000E-01,0.000000000000E+00,&
             0.000000000000E+00,-2.500000000000E-01,1.000000000000E+00,&
             -2.500000000000E-01,-2.500000000000E-01,2.500000000000E-01,&
             0.000000000000E+00/)

  gold = values

  call CompressCoefficients(values,map,indx1,N,j)

  exit_status = 0
  do i=1,N
    if (gold(i) /= values(indx1(i))) exit_status = 1
  enddo

  if (exit_status /= 0) print*,'TEST FAILED'
...
end subroutine UnitTests__Compression

subroutine CompressCoeffs_deprecated(k,res,map,coeffs,vec_size)
  !
  ! Given a vector of FEHM geom. coeffs, and assuming
  ! a linear 'pointer' mapping of (1,2,...,N-1,N), 
  ! then 'dedup' the coefficient vector and remap the
  ! pointers to the new mapping.
  !
  ! Function is O(n^2) time and should be optimized to 
  ! run in O(n).
  !
  ! USAGE:
  ! call compress_coefficients(kk,new,map,test,size(test))
  ! print*,map
  ! print*,new(1:kk)

  implicit none
  integer :: i, j, k, vec_size
  integer :: map(vec_size)
  real :: coeffs(vec_size)
  real :: res(vec_size)

  k = 1
  res(1) = coeffs(1)
  
  ! Iterate over the vector and remove duplicates
  ! (/1.,2.,1.,2./) -> (/1.,2./)
  do i=2,vec_size
    if (any( res == coeffs(i) )) cycle
    k = k + 1
    res(k) = coeffs(i)
  end do

  ! Iterate over the vector again, this time remapping the 'pointers'
  ! (/1,2,3,4/) -> (/1,2,1,2/)
  do i=1,vec_size
    do j=1,k
      if (coeffs(i) == res(j)) then
        map(i) = j
        cycle
      endif
    enddo
...
end subroutine CompressCoeffs_deprecated
...
subroutine idxToELNE(result,idx,length)
    ...
    do while (num > 0)
        ...
        num = (num - remainder) / real(permutations)
        i = i + 1
    enddo
...
#include "petsc/finclude/petscmat.h"
  ...
  PetscInt,  allocatable :: indx(:)
  PetscInt,  allocatable :: sendcounts(:),pos(:)
  PetscReal, allocatable :: connectivity(:,:),connectivity_local(:),connectivity_tmp(:),connect_area(:,:),connect_map(:),connect_map_local(:),temp_connections(:)
  Vec :: mat_diag
  PetscErrorCode :: ierr
  PetscViewer :: viewer
...
9150 format(I11,2X,I11,2X,E20.12,2X,E20.12,2X,E20.12,2X,E20.12)
 ...
 ! print to file 
  if (rank==io_rank) then
    ...
    connections_count = sum(connect_map)
...
subroutine DumpPLY(grid,rank,size)
  ! 
  ! Writes out a 2D or 3D Voronoi mesh as a 
...
#include "petsc/finclude/petscmat.h"
  ...
  character(len=50) :: nnodes,nfaces
  integer :: i,j,c(4)
  PetscInt :: rank, size, fileid
  PetscInt :: voronoi_slices
  PetscInt :: r,g,b,a
  PetscReal,dimension(:),allocatable :: v
  PetscReal :: v1(3),v2(3),v3(3),v4(3)

  allocate(v(3))

  voronoi_slices = grid%num_elems_global*3
  fileid = 1234561

  if (grid%ndim /= 2) then
    print*,'Only triangle .PLY output is currently supported.'
    return
  endif
  ...
  write(nnodes,'(I50)') 4*voronoi_slices!grid%num_pts_global
  write(nfaces,'(I50)') voronoi_slices!grid%num_elems_global

  ! Write out the PLY header
  write(fileid,'(a)') 'ply',&
  'format ascii 1.0',&
  'comment generated by VORONOI',&
  'comment (c) 2018 LANL',&
  'element vertex '//trim(adjustl(nnodes)),&
  'property float x',&
  'property float y',&
  'property float z',&
  'element face '//trim(adjustl(nfaces)),&
  ...
  ! write out nodes
  !   - connectivity vector! vor_cell_i connectivity count == non-NaN row entries == node out stride 

  ! TODO: Ensure that each face is 'right-handed'. This appears to be the cause of the 
  ! weird polygon renderings.

  ! for each row in grid%ply_m2...
  do i=1,voronoi_slices
    ! for each column in grid%ply_m2...
    do j=1,6,2
  ...
  do i=1,voronoi_slices
    a = 255
    r = 255*(real(grid%ply_m2(i,1))/real(grid%num_elems_global))
    g = r
    b = r
    j = (i-1)*4

    ! Below, we want to order the polygon vector in such
    ! a way that the right-hand rule is preserved.
    v1 = grid%ply_m1(grid%ply_m2(i,1),grid%ply_m2(i,2),:)
    v2 = grid%ply_m1(grid%ply_m2(i,3),grid%ply_m2(i,4),:)
    v3 = grid%ply_m1(grid%ply_m2(i,5),grid%ply_m2(i,6),:)
    v4 = grid%ply_m3(grid%ply_m2(i,7),:)

    ! Returns ordering as i in c for i in (1...4)
    call orderPointsInPolygon(c,v1,v2,v3,v4)
    ! Change c to represent current i in 0-notation
    c = (c + j - 1)
    
    write(fileid,'(9I4)') (/4,c(1),c(2),c(3),c(4),r,g,b,a/)
  enddo

  close(fileid)

  deallocate(grid%ply_m1)
  deallocate(grid%ply_m2)
  deallocate(grid%ply_m3)

end subroutine DumpPLY
...
#include "petsc/finclude/petscmat.h"
  ...
  PetscReal :: tmp, temp_real, area_local, face_area, vol
  ...
  character(len=50) :: l
  character(len=30) :: tmp_str,tmp_str2
  character(len=1) :: h1,h2
  character(len=20) :: element_type
  PetscInt :: w1,w2,i,j,k,ii,pos1,io_rank=0
  PetscInt :: icores,max_node_connections=0
  PetscErrorCode :: ierr

  ! Set up arrays to capture negative coefficients
  PetscReal, dimension(10) :: neg_vols_val, neg_coeff_val
  PetscInt, dimension(10) :: neg_vols_idx
  PetscInt, dimension(10,2) :: neg_coeff_idx

  PetscInt :: row_idx,ncols,max_connections,min_connections,max_connections_local,min_connections_local
  PetscInt, allocatable :: recvcounts(:),pos(:)
  PetscReal, pointer :: vec_ptr(:)
  PetscReal, allocatable :: vor_vol(:), temp_rank_storage1(:), temp_rank_storage2(:)
 ...
 ! enddo
  ...
  47 format(4X,A)
     ! - Text int: float
  48 format(8X,A46,i10,1x,A1,E15.7)
